              ...
              {open && (
                <div className="ml-[1.15rem] mt-1 flex flex-col gap-1 border-l border-border pl-4">
                  {visibleChildren.map((child) => {
                    // Longest match wins, so a group whose children nest —
                    // /reports and /reports/sales — highlights only the deepest
                    // one rather than both. Exact match always beats a prefix.
                    const matches = (href: string) => pathname === href || pathname.startsWith(href + '/')
                    const active =
                      matches(child.href) &&
                      !visibleChildren.some((o) => o.href !== child.href && o.href.length > child.href.length && matches(o.href))
