...
export const webhookEvents = pgTable(
  'webhook_events',
  {
    ...
    /**
     * The Razorpay SUBSCRIPTION this delivery concerned (migration 0079), for
...
// ── platform plans, entitlements & tenant subscriptions (M16, 0078) ──────────
...
export const plans = pgTable(
  'plans',
  {
    ...
    active: boolean('active').notNull().default(true),
    /**
     * The gateway these plan references belong to (migration 0079) — the
...
export const planEntitlements = pgTable(
  'plan_entitlements',
  {
     ...
     * 0078 so every reader's contract stays flat.
     */
...
export const tenantSubscriptions = pgTable(
  'tenant_subscriptions',
  {
     ...
     * flow (migration 0079): gateway = 'razorpay' on the PLATFORM's account.
     */
    gateway: text('gateway'),
     ...
     * cancel_at_cycle_end). NOT `cancelled_at`, which the 0078 CHECK ties to
    ...
    /**
     * Whether `currentPeriodStart/End` came from the PROVIDER (migration 0083).
     *
    ...
    /**
     * ── the dunning clocks (migration 0081) ─────────────────────────────────
     *
...
// ── dunning notices (migration 0081) ─────────────────────────────────────────
//
...
// ── the PLATFORM's own Razorpay account (migration 0079) ─────────────────────
//
...
// ── platform invoices: Arena OS → the business (migration 0080) ──────────────
//
...
//
// Money is numeric(10,2) rupees and GST-INCLUSIVE: 0079's subscribe path
...
export const platformInvoices = pgTable(
  'platform_invoices',
  {
    ...
    // by CHECK constraints in 0080 so a rounding bug fails the INSERT.
    ...
    /** A stored document, when one exists. Null today — see 0080 for why. */
...
// ── platform refunds (migration 0082) ────────────────────────────────────────
//
...
// (0080) CHECKs that a credit note carries no gateway payment, precisely so the
...
export const platformRefunds = pgTable(
  'platform_refunds',
  {
    ...
    /**
     * When the gateway confirmed the money left (migration 0084). Set ONCE, at
     ...
     * not on `updatedAt` (which moves on every write). See 0084.
     */
