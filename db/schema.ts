...
export const webhookEvents = pgTable(
  'webhook_events',
  {
    ...
    /**
     * The Razorpay SUBSCRIPTION this delivery concerned (migration 0080), for
...
// ── platform plans, entitlements & tenant subscriptions (M16, 0079) ──────────
...
export const plans = pgTable(
  'plans',
  {
    ...
    active: boolean('active').notNull().default(true),
    /**
     * The gateway these plan references belong to (migration 0080) — the
...
export const planEntitlements = pgTable(
  'plan_entitlements',
  {
     ...
     * 0079 so every reader's contract stays flat.
     */
...
export const tenantSubscriptions = pgTable(
  'tenant_subscriptions',
  {
     ...
     * flow (migration 0080): gateway = 'razorpay' on the PLATFORM's account.
     */
    gateway: text('gateway'),
     ...
     * cancel_at_cycle_end). NOT `cancelled_at`, which the 0079 CHECK ties to
    ...
    /**
     * Whether `currentPeriodStart/End` came from the PROVIDER (migration 0084).
     *
    ...
    /**
     * ── the dunning clocks (migration 0082) ─────────────────────────────────
     *
...
// ── dunning notices (migration 0082) ─────────────────────────────────────────
//
...
// ── the PLATFORM's own Razorpay account (migration 0080) ─────────────────────
//
...
// ── platform invoices: Arena OS → the business (migration 0081) ──────────────
//
...
//
// Money is numeric(10,2) rupees and GST-INCLUSIVE: 0080's subscribe path
...
export const platformInvoices = pgTable(
  'platform_invoices',
  {
    ...
    // by CHECK constraints in 0081 so a rounding bug fails the INSERT.
    ...
    /** A stored document, when one exists. Null today — see 0081 for why. */
...
// ── platform refunds (migration 0083) ────────────────────────────────────────
//
...
// (0081) CHECKs that a credit note carries no gateway payment, precisely so the
...
export const platformRefunds = pgTable(
  'platform_refunds',
  {
    ...
    /**
     * When the gateway confirmed the money left (migration 0085). Set ONCE, at
     ...
     * not on `updatedAt` (which moves on every write). See 0085.
     */
