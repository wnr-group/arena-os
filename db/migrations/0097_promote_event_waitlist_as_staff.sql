-- ============================================================================
-- Arena OS — 0097 staff waitlist promotion entry point (M15 #5 §7)
--
...
--
-- `promote_event_waitlist()` already exists (0091) and already does the whole
...
--
-- But 0091 revoked it from `public` and never granted it to `arena_app`, on
...
-- 0091's own entry points use: SECURITY DEFINER, authorises from the SESSION
...
comment on function public.promote_event_waitlist_as_staff(uuid) is
  'Manager-authorised waitlist promotion for no-show recovery (M15 #5). Wraps promote_event_waitlist() (0091) — which owns every capacity, FIFO and paid-event rule — with an auth_is_manager() check against the EVENT''s tenant and a per-event lock. Returns the number promoted; returns 0 for a non-manager and for an unknown event alike, so event ids cannot be probed.';
