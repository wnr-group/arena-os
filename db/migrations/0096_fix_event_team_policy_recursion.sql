-- ============================================================================
-- Arena OS — 0096 fix infinite recursion in the event team policies
--
-- A CORRECTNESS FIX to 0091 (M15 #3), found while building day-of check-in
-- (M15 #5). Not a feature.
--
-- ══ THE BUG ═════════════════════════════════════════════════════════════════
--
-- 0091 gave `event_team_members` two policies that read `event_team_members`:
--
...
comment on function public.customer_team_ids() is
  'Teams the current OTP-session customer belongs to. SECURITY DEFINER so the event_teams / event_team_members policies can test membership without re-entering the table they guard — the same reason auth_tenant_ids() exists for memberships (0002). Added by 0096 to fix the recursion 0091 introduced.';
