-- ============================================================================
-- Arena OS — 0081 dunning & suspension on failed payment (AROS-113)
--
-- 0078 built the subscription MODEL and said it "does NOT add: checkout,
-- gateway subscription creation, renewals, dunning". 0079 built checkout and
...
--
-- — is already fully expressible with the two enums this schema has, and 0079
...
--
-- "suspended" is an ACCOUNT state, not a subscription state — 0079 states the
...
--
-- Modelled on webhook_events (0034/0079), the delivery log this codebase
...
create table if not exists public.platform_dunning_notices (
  ...
  -- restrict, like platform_invoices.subscription_id (0080): the record of what
...
-- ── RLS + grants ────────────────────────────────────────────────────────────
--
-- Exactly the treatment platform_invoices got in 0080, for exactly the same
...
-- helper 0020's business_profiles and 0080's platform_invoices use.
--
...
-- tenant_subscriptions keeps the grants 0078 gave it — SELECT only to
...
-- suspended_at any more than it could clear its own status. And 0079 already
