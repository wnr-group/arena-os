-- ============================================================================
-- Arena OS — 0052 platform invoices: recurring billing + GST (M16 #4)
--
-- 0050 built the plan/entitlement model. 0051 connected Arena OS's own Razorpay
...
--
-- numeric(10,2) rupees everywhere, matching `invoices` (0018), `plans` (0050)
...
--
-- PLAN PRICES ARE GST-INCLUSIVE. This is forced, not chosen: 0051's
...
--
-- A singleton, the same `id boolean` idiom as platform_payment_settings (0051):
...
create table if not exists public.platform_invoices (
  ...
  -- an invoice references must not be deletable out from under it. 0050 already
  ...
  -- tenant_subscriptions (0050) and payment_intents (0033).
  gateway                 text,
...
-- 0051's subscribe/cancel actions are behind requireOwner().
--
...
--
-- No policies and no grants, exactly like platform_payment_settings (0051).
