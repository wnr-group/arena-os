 ...
 * `platform_invoices_owner_select` (0052) admits only invoices for a tenant the
 * caller OWNS, `tenant_subscriptions_select` (0050) only their own
...
export type PortalInvoice = {
  ...
  currency: string
  status: string
  /** A stored document, when one exists. Null today — see migration 0052. */
...
export async function getBillingPortal(ctx: ActiveContext): Promise<BillingPortal> {
  return withUser(ctx.user.id, async (tx) => {
    ...
    //
    // RLS (tenant_subscriptions_select, 0050) confines this to the caller's own
    ...
    const portalPlans: PortalPlan[] = catalogue
      ...
      // policy plans_select_subscribed, 0050) but must never be offered.
      .filter((p) => p.active)
      .map((p) => ({
        id: p.id,
        name: p.name,
