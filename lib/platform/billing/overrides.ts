 ...
 *
 * The same three statuses `idx_tenant_subscriptions_one_live` (0050) permits,
 ...
 * It supports credits, fully, and has since AROS-4 (migration 0052):
 *
 ...
 * and already exists. 0051's header states the same separation from the other
...
export async function forceCancelTenantSubscription(
  ...
  return db.transaction(async (tx) => {
    ...
    await tx
      ...
      .set({
        status: 'cancelled',
        // tenant_subscriptions_cancelled_at (0050) CHECKs that this is set if
