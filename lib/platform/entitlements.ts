...
/** A JSON scalar. The check constraint in 0050 is what keeps it flat. */
 ...
 *
 * Deliberately the SAME three the partial unique index in 0050
 ...
 * (migration 0053), read below. current_period_end keeps meaning exactly what
 * it always meant.
 */
 ...
 * filter on plans.active here — and policy `plans_select_subscribed` in 0050
...
export async function readEntitlements(tx: DB, tenantId: string): Promise<TenantEntitlements> {
  ...
  // before migration 0053 backfilled them, or by a future path that forgets to
 ...
 * (tenant_subscriptions_select, 0050) independently confines the query to the
