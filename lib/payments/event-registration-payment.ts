 ...
 * INSERT policy on payment_intents (migration 0093) contains
 *
 ...
 * context has no UPDATE policy on event_registrations (by design — see 0091).
...
async function loadPayableRegistration(
  ...
  if (!row.holdExpiresAt || row.holdExpiresAt.getTime() <= Date.now()) {
    ...
    // INSERT policy in 0093 would refuse the intent anyway.
    throw new EventRegistrationPaymentError(
