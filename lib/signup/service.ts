 ...
 *   * plan    — idx_tenant_subscriptions_one_live (0050) permits exactly one
 ...
 * `gateway_subscription_id` left null, which is precisely what 0050 says an
  ...
  return db.transaction(async (tx) => {
    ...
    const [row] = await tx
      ...
      .values({
        ...
        // The live-but-unpaid state 0050 already defines. Deliberately NOT a
